...
# DAILY prediction (commented out)
...
# Log predictions
log_path = "/opt/loxberry/data/plugins/consumption_prediction/prediction.log"
...
for dt, pred in zip(prediction_data['datetime'], predictions):
    cursor.execute("INSERT INTO predictions (datetime, predicted_kwh) VALUES (?, ?)", (dt.strftime('%Y-%m-%d %H:%M:%S'), pred))
conn.commit()
conn.close()

# Save to DB (daily total) — commented
# cursor = conn.cursor()
# cursor.execute("CREATE TABLE IF NOT EXISTS predictions (id INTEGER PRIMARY KEY AUTOINCREMENT, datetime TEXT NOT NULL, predicted_kwh REAL NOT NULL);")
...
# Send hourly predictions to MQTT
